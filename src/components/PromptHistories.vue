<template>
    ...
    <div v-if="loading" class="flex flex-col h-full space-y-3">
        <USkeleton class="h-4 w-[250px]" style="--ui-bg-elevated: #ccc" />
        <USkeleton class="h-4 w-[200px]" style="--ui-bg-elevated: #ccc" />
    </div>
    <div v-else-if="promptHistory.length > 0">
        <div>

            <div v-for="(item, index) in promptHistory" :key="index"
                ...
                <div class="flex gap-2 ">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-gray-500 line-clamp-2 mb-0">{{ item.finalPrompt }}</p>
                        <a @click.stop :href="`http://taoprompt.com/p/${item.id}`" target="_blank"
                            class="text-primary text-xs flex items-center gap-1">
                            View
                            <UIcon name="famicons:open-outline" class="size-3" />
                        </a>
                    </div>
                </div>
            </div>
            <div v-if="!isEnd" class="flex items-center justify-center">
                <UButton @click="loadMore" color="primary" variant="ghost" size="sm" block class="cursor-pointer"
                    :disabled="loadmoreLoading" :loading="loadmoreLoading">
                    <span class="text-gray-400">{{ $t('more') }}</span>
                </UButton>
            </div>
        </div>
    </div>
    <div v-else class="flex flex-col items-center justify-center h-full max-w-[12rem] mx-auto space-y-3">
        <Icon name="mingcute:empty-box-line"
        ...
        <p class="text-center text-sm text-gray-400 dark:text-white/40">{{ $t('no-prompt') }}</p>
    </div>
</template>
...
const getPromptList = async (searchQuery: string, page: number, limit: number) => {
    const input = {
        params: {
            q: searchQuery,
            page,
...
const fetchPromptHistory = async () => {
    ...
    if (page.value === 1) {
        ...
        promptHistory.value.push(...res.prompts.filter((item: any) => item.title !== ''))
    }
